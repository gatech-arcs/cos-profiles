...
class FastlySettingsForm extends ConfigFormBase {
   ...
   */
  public function __construct(ConfigFactoryInterface $config_factory, TypedConfigManagerInterface $typed_config_manager, Api $api, State $state, VclHandler $vclHandler, Webhook $webhook, RequestStack $requestStack) {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['account_settings']['site_id'] = [
      ...
      '#default_value' => $config->get('site_id') ?? getenv('FASTLY_SITE_ID'),
      '#disabled' => getenv('FASTLY_SITE_ID') ? TRUE : FALSE,
      '#required' => FALSE,
      '#description' => $this->t("(Use letters, numbers, hyphens and underscores only). Site identifier which is being prepended to cache tags. Use this if you have multiple sites in the same service in Fastly. Note: You can use the environment variable <code>FASTLY_SITE_ID</code> to set this also. If nothing is set in either config or environment variable then one will be randomly generated for you."),
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    // Ensure site_id only contains valid characters.
    $siteId = $form_state->getValue('site_id');
    if (!empty($siteId) && !preg_match('/^[a-zA-Z0-9_-]+$/', $siteId)) {
      $form_state->setErrorByName('site_id', $this->t('Site ID can only contain letters, numbers, hyphens, and underscores.'));
    }
