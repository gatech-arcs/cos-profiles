module.exports = {
  ...
  before(browser) {
    browser
      ...
      .drupalLoginAsAdmin(() => {
        browser
          // Enable JavaScript aggregation so that we can ensure that the paste
          // filter plugin is aggregated properly.
          .drupalRelativeURL('/admin/config/development/performance')
          .click('[name="preprocess_js"]')
          .assert.attributeEquals('[name="preprocess_js"]', 'checked', 'true')
          .submitForm('input#edit-submit') // Submit config form.
  ...
  'Test paste filter default settings': (browser) => {
    browser.drupalLoginAsAdmin(() => {
      browser
        ...
        // Check that there is not a separate aggregate for our plugin, it
        // should be aggregated with other CKEditor 5 plugins.
        .assert.not.elementPresent('script[src*="build/pasteFilter.js"]');
    });
  },
};
