...
class CasGatewayAuthSubscriberTest extends CasBrowserTestBase {
   ...
   */
  public function testGatewayPaths(): void {
    ...
    $edit = [
      ...
      'gateway[method]' => GatewayMethod::ServerSide->value,
    ];
    ...
    $this->assertEquals(GatewayMethod::ServerSide->value, $config->get('gateway.method'));
    ...
    $edit = [
      'gateway[enabled]' => TRUE,
      'gateway[recheck_time]' => "-1",
      'gateway[method]' => GatewayMethod::ClientSide->value,
    ];
    $this->drupalGet('/admin/config/people/cas');
    $this->submitForm($edit, 'Save configuration');
    ...
    // to CAS via server-side, but the page contains the JS library to redirect.
    ...
    // @todo Test that visiting page as a bot does NOT trigger a redirect.
