...
class CasForcedAuthSubscriberTest extends CasBrowserTestBase {
   ...
   */
  public function testForcedLoginPaths(): void {
    global $base_path;
    ...
    $this->assertEquals(302, $session->getStatusCode());
    $expected_redirect_url = 'https://fakecasserver.localhost/auth/login?' . UrlHelper::buildQuery(['service' => $this->buildServiceUrlWithParams(['destination' => $base_path . 'my/path?destination=' . urlencode('/some/other/path') . '&foo=bar'])]);
    $this->assertEquals($expected_redirect_url, $session->getResponseHeader('Location'));
