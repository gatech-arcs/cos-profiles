...
class CasUserFormFieldTest extends CasBrowserTestBase {
   ...
   */
  public function testCasUsernameField(): void {
    ...
    $this->assertEquals('test_user_1_cas', $authmap->get((int) $test_user_1->id(), 'cas'));
    ...
    $this->submitForm($edit, 'Save');
    $authmap = $this->container->get('externalauth.authmap');
    $this->assertFalse($authmap->get((int) $test_user_2->id(), 'cas'));
   ...
   */
  public function testRestrictedPasswordManagementWorks(): void {
   ...
   */
  public function testRestrictedEmailManagementWorks(): void {
    $admin = $this->drupalCreateUser([
      'administer account settings',
      'administer users',
    ]);
    ...
    $email_field = $page->findField('mail');
    $this->assertNotNull($email_field);
    $this->assertObjectNotHasProperty('disabled', $email_field);
