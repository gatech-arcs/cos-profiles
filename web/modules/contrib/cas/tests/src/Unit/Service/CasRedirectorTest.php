...
class CasRedirectorTest extends UnitTestCase {
   ...
   */
  protected EventDispatcherInterface|MockBuilder $eventDispatcher;

  /**
   * The mocked config factory service.
   */
  protected ConfigFactoryInterface|MockBuilder $configFactory;
  ...
  public function setUp(): void {
    ...
    $this->casHelper->method('getCasSettings')->willReturn($this->configFactory->get('cas.settings'));
   ...
   */
  public function getServiceUrl(string $route, ?array $parameters = NULL): string {
    if ($parameters) {
   ...
   *   The event instance.
   */
  public function dispatchEvent(CasPreRedirectEvent $event): CasPreRedirectEvent {
   ...
   */
  public function testBuildRedirectResponse(): void {
    ...
    // Verify setting of gateway parameters.
    ...
    $this->assertSame(['config:cas.settings'], $tags);

    // Simulate a config override that adds an additional cache tag.
    // This happens in e.g. cas_mock_server.
    $config = $this->casHelper->getCasSettings();
    assert($config instanceof MockObject);
    $config->method('getCacheTags')
      ->willReturn(['config:cas.settings', 'custom_cache_tag']);

    $response = $cas_redirector->buildRedirectResponse($cas_data);
    $this->assertInstanceOf(TrustedRedirectResponse::class, $response);
    $tags = $response->getCacheableMetadata()->getCacheTags();
    // In Drupal < 11.1, array keys for merged tags are not normalized.
    /* @see \Drupal\Core\Cache\Cache::mergeTags() */
    $tags = array_values($tags);
    $this->assertSame(['config:cas.settings', 'custom_cache_tag'], $tags);
  ...
  public function testEventsDispatched(): void {
    ...
    // Verify that the event fired and the redirector was appropriately altered.
