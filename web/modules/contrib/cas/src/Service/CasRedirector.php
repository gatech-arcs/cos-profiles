...
class CasRedirector {
   ...
   */
  public function buildRedirectResponse(CasRedirectData $data, bool $force = FALSE): TrustedRedirectResponse|CasRedirectResponse|null {
    $response = NULL;

    $config = $this->casHelper->getCasSettings();
    $casServerConfig = CasServerConfig::createFromModuleConfig($config);
    ...
    $this->eventDispatcher->dispatch($pre_redirect_event);
    $this->eventDispatcher->dispatch($pre_redirect_event, 'cas.pre_redirect');
