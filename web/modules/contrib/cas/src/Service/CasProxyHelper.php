...
class CasProxyHelper {
   ...
   */
  private function getServerProxyUrl(string $target_service): string {
   ...
   */
  public function getProxyTicket(string $target_service): string {
    if (!$this->casHelper->getCasSetting('proxy.initialize') || !$this->session->has('cas_pgt')) {
   ...
   *
   * Returns cookies from the proxied service in a CookieJar object for use when
   * later accessing resources.
   *
   * @param string $target_service
   *   The service to be proxied.
   *
   * @return \GuzzleHttp\Cookie\CookieJarInterface
   *   A CookieJar object (array storage) containing cookies from the proxied
   *   service.
   *
   * @throws \Drupal\cas\Exception\CasProxyException|\GuzzleHttp\Exception\GuzzleException
   *   Thrown if there was a problem communicating with the CAS server
   *   or if there was an invalid use of session data.
   */
  public function proxyAuthenticate(string $target_service): CookieJarInterface {
    ...
    try {
      ...
      $this->httpClient->get($service_url, [
        'cookies' => $cookie_jar,
        'timeout' => $this->casHelper->getCasSetting('advanced.connection_timeout'),
      ]);
    }
    catch (ClientException $e) {
      throw new CasProxyException($e->getMessage());
   ...
   */
  private function parseProxyTicket(string $xml): mixed {
    $dom = new \DOMDocument();
   ...
   */
  public function storePgtSession(string $pgt_iou): void {
    ...
    if ($pgt) {
      $this->session->set('cas_pgt', $pgt->pgt);
    }
