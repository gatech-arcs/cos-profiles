...
class CasLoginRedirectSubscriber implements EventSubscriberInterface {
  ...
  public function onRedirectResponse(ResponseEvent $event): void {
    ...
    if (in_array($route_match->getRouteName(), ['cas.login', 'cas.legacy_login'], TRUE)) {
      ...
      // we redirect to.
      // We do it in a response subscriber instead of ForceLoginController
      // because we need to remove the destination param even for cached
      // DynamicPageCache responses, which don't execute our controller code.
      // Take no action if CasRedirectLoggedInUsersSubscriber indicated it
      // should be preserved.
      // @see \Drupal\Core\EventSubscriber\RedirectResponseSubscriber::checkRedirectUrl()
      // @see \Drupal\cas\Controller\ForceLoginController::forceLogin()
      if ($event->getRequest()->query->has('destination') && !$event->getRequest()->attributes->has('cas_preserve_destination_param')) {
